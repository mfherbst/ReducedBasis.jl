...
# In [Greedy basis assembly using DMRG](@ref) we have seen that we can customize the snapshot solvers as well as the compression methods during reduced basis assembly.
...
# Since we do not compute any Hamiltonian compressions during POD, we need to
# compute them afterwards using the [`HamiltonianCache`](@ref) constructor
# (recall that `h` is needed in the online stage):
...
magnetization = map(grid_online) do μ
    ...
    mean(eachcol(φ_rb)) do u
        abs(dot(u, m_reduced, u))
    end
end

# With this we can again produce a magnetisation diagram:
