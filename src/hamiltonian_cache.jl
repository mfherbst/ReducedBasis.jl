...
struct HamiltonianCache{M}
    H::AffineDecomposition
    HΨ::Vector{M}  # Hard-code to Vector for now (generalized structure even needed?)
...
function HamiltonianCache(H::AffineDecomposition, basis::RBasis)
    HΨ = [term * basis.snapshots for term in H.terms]  # TODO: generalize to MPO/MPS
...
function HamiltonianCache(
    H::AffineDecomposition, basis::RBasis{T,P,MPSColumns{T},N}
) where {T<:Number,P<:AbstractVector,N<:Union{UniformScaling,AbstractMatrix{T}}}
    HΨ = [apply(term, basis.snapshots) for term in H.terms]
    ΨHΨ = [overlap_matrix(basis.snapshots.cols, v) for v in HΨ]
    ΨHHΨ = reshape([overlap_matrix(v1, v2) for v1 in HΨ for v2 in HΨ], (n_terms(H), n_terms(H)))
    h = AffineDecomposition([basis.vectors' * matel * basis.vectors for matel in ΨHΨ], H.coefficient_map)
    h² = AffineDecomposition(
        [basis.vectors' * matel * basis.vectors for matel in ΨHHΨ],
        μ -> (H.coefficient_map(μ) * H.coefficient_map(μ)')
...
function extend!(hc::HamiltonianCache, basis::RBasis,_)
...
function extend!(hc::HamiltonianCache{Vector{MPS}}, basis::RBasis; kwargs...)
    d_basis = dimension(basis)
    m = multiplicity(basis)[end]  # Multiplicity of last truth solve
    ...
    for q in eachindex(hc.HΨ)
        for j = d_basis-m+1:d_basis
            push!(hc.HΨ[q], apply(hc.H.terms[q], basis.snapshots.cols[j]; kwargs...))
        end
    end
    ...
    for (q, term) in enumerate(hc.ΨHΨ)
        ...
        for j = d_basis-m+1:d_basis
            for i = 1:j
                term_new[i, j] = dot(basis.snapshots.cols[i], hc.HΨ[q][j])
                term_new[j, i] = term_new[i, j]'
            end
        end
        hc.ΨHΨ[q] = term_new
